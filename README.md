...
**자동화 (이중 계층)**:
- **1차(신뢰 계층) — Windows 작업 스케줄러**: `scripts/surge_daily.ps1`
  - `surge-daily-morning` 화~토 07:13 KST(미국 마감 후): snapshot→backfill→fade→duel-eval
  - `surge-daily-evening` 월~금 21:35 KST(아시아 마감 후·미국 개장 전): duel 콜 생성
  - `StartWhenAvailable` — 부팅/로그인 시 놓친 실행 자동 보충. 로그: `data/logs/`
  - 해제: `Unregister-ScheduledTask -TaskName surge-daily-morning,surge-daily-evening`
- **2차(보고/폴백) — 앱 내 작업**: `daily-surge-snapshot`(07:43, OS 잡 결과 확인·누락 시 대행),
  `nightly-duel-call`(21:50, 오늘 밤 콜 카드 보고). 앱이 켜져 있을 때만 동작.
- 전제: 해당 시각에 PC가 켜져 있고 로그인돼 있어야 한다.
